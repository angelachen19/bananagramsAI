...
def checkwords():
    ...
    for ii in range(COLS):
        for jj in range(ROWS):
            if array[ii][jj] != ' ' and array[ii][jj] != '_':
...
def drawconsole():
    ...
    draw.rect(screen, [60, 60, 60], (framehor-3, framever -
              3 + SIZE*(ROWS+1), SIZE*COLS+7, SIZE*2+7), 3)
    draw.rect(screen, [60, 60, 60], (framehor, framever +
...
def drawgameboard():
    global wrongfade
    draw.rect(screen, [60, 60, 60], (framehor-3,
    ...
    if len(invalid) > 0:
        ...
        if wrongfade < 0:
            wrongfade = 0
        for cell in invalid:
            draw.rect(screen, [60 + wrongfade, 60, 60], (framehor +
                      cell[0]*SIZE, framever + cell[1]*SIZE, SIZE, SIZE), 2)
            if wrongfade == 0:
                invalid.remove(cell)
    if grabbed != ' ':
        draw.rect(screen, [60, 60, 60], (framehor + mousex + 1 -
                  offsetx, framever + mousey + 1 - offsety, SIZE, SIZE), 1)
        draw.rect(screen, [60, 60, 60], (framehor + mousex + 2 -
                  offsetx, framever + mousey + 2 - offsety, SIZE, SIZE), 1)
        draw.rect(screen, tilecolor(grabbed), (framehor + mousex -
                  offsetx, framever + mousey - offsety, SIZE, SIZE))
        myfont = font.SysFont("Times New Roman", SIZE - 3)
        label = myfont.render(grabbed, 1, [0, 0, 0])
        screen.blit(label, (framehor + mousex - offsetx -
                    ((myfont.size(grabbed))[0] - SIZE)/2, framever + mousey + 1 - offsety))
        draw.rect(screen, [60, 60, 60], (framehor + mousex -
                  offsetx, framever + mousey - offsety, SIZE, SIZE), 2)
#####################################################################
...
COLS = 18
SIZE = 40
array = [[' ']*ROWS for x in range(COLS)]  # format is array[column][row]
invalid = []
...
screen = display.set_mode(
    [screenwidth + framehor*2, screenheight + framever*5])
...
letterfreq = [7, 2, 2, 3, 9, 2, 2, 1, 6, 1, 1,
              2, 2, 4, 6, 2, 1, 4, 3, 4, 3, 1, 1, 1, 1, 1]
letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
letterbag = ""
for ii in range(len(letters)):
    letterbag += letterfreq[ii] * letters[ii]
...
def peelletter():
    ...
    randLetter = random.choice(newletterbag)
    newletterbag = newletterbag.replace(randLetter, "", 1)
    ...
    for myevent in event.get():  # User did something
        try:
            ...
            if myevent.type == MOUSEBUTTONDOWN and grabbed == ' ' and waitmouse == 0:
                ...
                if mousex < 0 or mousex > screenwidth:
                    continue
            ...
            if myevent.type == MOUSEBUTTONDOWN and grabbed != ' ' and waitmouse == 0:
                ...
                mousex = myevent.pos[0] - framehor
                mousey = myevent.pos[1] - framever
                if mousex + SIZE/2 - offsetx < 0 or mousex + SIZE/2 - offsetx > screenwidth:
                    continue
            ...
            if myevent.type == KEYDOWN:
                ...
                if myevent.key == K_RIGHT:
                    temp = array[COLS - 1]
                    for ii in range(COLS - 1):
                        array[COLS - ii - 1] = array[COLS - ii - 2]
                    array[0] = temp
                if myevent.key == K_UP:
                    temp = [0]*COLS
                    for ii in range(COLS):
                    ...
                    for ii in range(COLS):
                        array[ii][ROWS-1] = temp[ii]
                if myevent.key == K_DOWN:
                    temp = [0]*COLS
                    for ii in range(COLS):
                        temp[ii] = array[ii][ROWS-1]
                    for ii in range(COLS):
                        for jj in range(ROWS - 1):
